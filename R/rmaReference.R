...
rmaReference <- function(affy.batch, reference, test = FALSE) {
  ...
  if (contin) {
    tmp <- userRMA(ref.pm , probesets, colnames(ref.pm),
                   reference$quantile,  reference$alpha[rownames(ref.pm)])

    # Scale the rma normalised
    tmp$exprs.sc <- (tmp$exprs - reference$median[rownames(tmp$exprs)]) /
      reference$sd[rownames(tmp$exprs)]

    tmp$exprs.sc.mean <- (tmp$exprs - reference$mean[rownames(tmp$exprs)]) /
      reference$sd[rownames(tmp$exprs)]
